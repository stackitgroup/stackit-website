<section class="min-h-dvh flex items-center bg-black  ">
  <div class="container mx-auto px-6 text-center">
    <div class="max-w-5xl mx-auto">
      <h1 class="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black text-white leading-tight mb-6 fade-in-section">
        Your External Dev Team with
        <span class="gradient-text">In-House Drive</span>
      </h1>
      <p
        class="text-xl sm:text-2xl md:text-3xl text-gray-300 max-w-4xl mx-auto fade-in-section"
        style="transition-delay: 150ms">
        Build extraordinary software with a partner who shares your
        standards
      </p>
      <div class="mt-16 text-center fade-in-section">
        <a
        href="#contact"
        class="bg-[#3F5FDD] text-white font-semibold px-12 py-4 text-lg sm:px-24 sm:py-6 sm:text-xl hover:bg-[#3550B8] transition-colors duration-300 inline-block  w-full md:w-auto rounded-lg">Get
        Started</a>
      </div>
    </div>
  </div>
</section>